...
class ColumnInspector
{
    ...
    {
        $rawType = strtolower($row['Type']); // e.g. "varchar(255)", "tinyint(1)", "enum('a','b')"
        $type    = preg_replace('/\(.*\)/', '', $rawType); // strip length/values → "varchar"
    ...
    private function getSqliteColumns(string $table): array
    {
        ...
        foreach ($rows as $row) {
            $rawType = strtolower($row['type']); // e.g. "integer", "text", "real", "varchar(100)"
